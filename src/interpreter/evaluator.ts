import { Node, Program, ExpressionStatement, IntegerLiteral, InfixExpression, IfExpression, WhileExpression, BlockStatement, AssignStatement, Identifier, FunctionLiteral, CallExpression, PipelineExpression, StringLiteral, Expression, ArrayLiteral, IndexExpression, HashLiteral } from "../ast/ast";
...
import { CeledDiscordBot } from '../discord/discord';
...
const builtins: Record<string, Builtin> = {
    ...
    "file_read": new Builtin(async (...args: Obj[]) => {
        if (args.length !== 1) return new ErrorObj(`wrong number of arguments. got=${args.length}, want=1`);
        if (!(args[0] instanceof StringObj)) return new ErrorObj(`argument to \`file_read\` must be STRING, got ${args[0].type()}`);
        try {
            const data = fs.readFileSync(args[0].value, 'utf-8');
            return new StringObj(data);
        } catch (e: any) {
            return new ErrorObj(`file_read error: ${e.message}`);
        }
    }),
    "file_write": new Builtin(async (...args: Obj[]) => {
        if (args.length !== 2) return new ErrorObj(`wrong number of arguments. got=${args.length}, want=2`);
        if (!(args[0] instanceof StringObj)) return new ErrorObj(`first argument to \`file_write\` must be STRING, got ${args[0].type()}`);
        if (!(args[1] instanceof StringObj)) return new ErrorObj(`second argument to \`file_write\` must be STRING, got ${args[1].type()}`);
        try {
            fs.writeFileSync(args[0].value, args[1].value, 'utf-8');
            return nativeBoolToBooleanObject(true);
        } catch (e: any) {
            return new ErrorObj(`file_write error: ${e.message}`);
        }
    }),
    "discord": new Builtin(async (...args: Obj[]) => {
        if (args.length !== 1 || !(args[0] instanceof StringObj)) return new ErrorObj(`discord requires 1 STRING argument (token)`);
        const token = args[0].value;
        const bot = new CeledDiscordBot(token);
        
        const pairs = new Map<string, HashPair>();
        
        const connectFn = new Builtin(async (...cargs: Obj[]) => {
            bot.connect();
            return NULL;
        });
        pairs.set("connect", new HashPair(new StringObj("connect"), connectFn));
        
        const onMessageFn = new Builtin(async (...margs: Obj[]) => {
            if (margs.length !== 1 || !(margs[0] instanceof FunctionObj)) return new ErrorObj(`onMessage requires 1 FUNCTION argument`);
            const fnObj = margs[0];
            bot.onMessage((msgContent: string) => {
                const extendedEnv = new Environment(fnObj.env);
                if (fnObj.parameters.length > 0) {
                    extendedEnv.set(fnObj.parameters[0].value, new StringObj(msgContent));
                }
                evaluate(fnObj.body, extendedEnv).catch(console.error);
            });
            return NULL;
        });
        pairs.set("onMessage", new HashPair(new StringObj("onMessage"), onMessageFn));
...
export async function evaluate(node: Node | null, env: Environment): Promise<Obj> {
    ...
    if (node instanceof WhileExpression) return await evalWhileExpression(node, env);
    ...
    if (node instanceof InfixExpression) {
        ...
        return await evalInfixExpression(node.operator, left, right);
    }
...
async function evalInfixExpression(operator: string, left: Obj, right: Obj): Promise<Obj> {
    if (operator === "&&") return nativeBoolToBooleanObject(isTruthy(left) && isTruthy(right));
    if (operator === "||") return nativeBoolToBooleanObject(isTruthy(left) || isTruthy(right));
...
async function evalWhileExpression(we: WhileExpression, env: Environment): Promise<Obj> {
    let result: Obj = NULL;
    while (true) {
        const condition = await evaluate(we.condition, env);
        if (isError(condition)) return condition;
        if (!isTruthy(condition)) break;
        result = await evaluate(we.body, env);
        if (result && (result.type() === "RETURN_VALUE" || result.type() === "ERROR")) {
            return result;
        }
    }
