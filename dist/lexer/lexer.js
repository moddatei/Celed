...
class Lexer {
        ...
        switch (this.ch) {
            ...
            case "|":
                if (this.peekChar() === ">") {
                    ...
                    const char = this.ch;
                    this.readChar();
                    tok = { type: token_1.TokenType.OR, literal: char + this.ch };
                }
                else {
                    tok = { type: token_1.TokenType.ILLEGAL, literal: this.ch };
                }
            ...
            case "&":
                if (this.peekChar() === "&") {
                    const char = this.ch;
                    this.readChar();
                    tok = { type: token_1.TokenType.AND, literal: char + this.ch };
                }
                else {
                    tok = { type: token_1.TokenType.ILLEGAL, literal: this.ch };
                }
            ...
            case "!":
                if (this.peekChar() === "=") {
                    const char = this.ch;
                    this.readChar();
                    tok = { type: token_1.TokenType.NOT_EQ, literal: char + this.ch };
                }
                else {
                    tok = { type: token_1.TokenType.ILLEGAL, literal: this.ch };
                }
            ...
            case "/":
                ...
                break;
            case "%":
                tok = { type: token_1.TokenType.MOD, literal: this.ch };
                break;
            case ".":
                tok = { type: token_1.TokenType.DOT, literal: this.ch };
                break;
            case "<":
                if (this.peekChar() === "=") {
                    const char = this.ch;
                    this.readChar();
                    tok = { type: token_1.TokenType.LTE, literal: char + this.ch };
                }
                else {
                    tok = { type: token_1.TokenType.LT, literal: this.ch };
                }
                break;
            case ">":
                if (this.peekChar() === "=") {
                    const char = this.ch;
                    this.readChar();
                    tok = { type: token_1.TokenType.GTE, literal: char + this.ch };
                }
                else {
                    tok = { type: token_1.TokenType.GT, literal: this.ch };
                }
                break;
            case "{":
    ...
    lookupIdent(ident) {
        switch (ident) {
            ...
            case "while": return token_1.TokenType.WHILE;
